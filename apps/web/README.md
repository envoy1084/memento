...
`routes/` defines file routes; the Router plugin owns `routeTree.gen.ts`. `features/` composes individual journeys. `components/` contains product compositions built with UIKit. `atoms/` owns preview state and deterministic transitions; `hooks/` adapts atoms to React. Runtime schemas live in `@memento/protocol`.
