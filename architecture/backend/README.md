...
Creating a gift prepares an email-bound gift and an atomic USDC approval/deposit batch. Funding confirmation checks the successful onchain gift before marking it ready and inserting a deduplicated email job in the same database transaction. The sender UI does not expose manual invitation copying or delivery. The worker delivers to the encrypted saved contact; development logs the message, production uses Resend.
