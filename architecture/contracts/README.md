...
The shared public manifest is `packages/chain/src/deployments/sepolia.json`. ENS dependencies come from ENSForge’s Sepolia v2 deployment. `sponsorship` is intentionally null until the operator deploys this version. Startup validates bytecode and configured bindings. Do not use the previous escrow address with this ABI.
